...
isminetype IsMine(const CKeyStore &keystore, const CScript& scriptPubKey)
{
    ...
    case TX_ZEROCOIN:
    {
        CPubKey p(vSolutions[0]); CBigNum c(vSolutions[1]);  CBigNum a(vSolutions[2]);
        ...
        if(!keystore.GetBlindingCommitment(bc))
            break;
        if(libzerocoin::PrivateCoin::QuickCheckIsMine(&Params().GetConsensus().Zerocoin_Params, zk, p, bc, c, a)) {
